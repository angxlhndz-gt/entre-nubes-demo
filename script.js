...
function renderHome() {
  return `<div class="home-page">
    ...
    <section class="section-pad home-experiences"><div class="section-heading reveal"><div><p class="eyebrow">Elige tu manera de estar aquí</p><h2>Experiencias</h2></div><p class="section-note">Cada experiencia se coordina directamente con la finca.</p></div><div class="experience-grid">${EXPERIENCE_CARDS.map((card, i) => `<a class="experience-card reveal ${i === 0 ? 'experience-card-tall' : ''} ${i === 5 ? 'experience-card-wide' : ''}" href="${card.href}"><img src="${card.image}" alt="${card.title} en Entre Nubes y Montañas" loading="lazy"><span class="experience-overlay"></span><span class="card-content"><span class="card-kicker">${card.kicker}</span><h3>${card.title}</h3><p>${card.copy}</p><span class="card-link">Explorar <span aria-hidden="true">↗</span></span></span></a>`).join('')}</div></section>

    <section class="cabins section-pad"><div class="section-heading reveal"><div><p class="eyebrow">Quédate entre las montañas</p><h2>Cabañas</h2></div><p class="section-note">Capacidades para grupos familiares. Consulta disponibilidad y condiciones por WhatsApp.</p></div><div class="cabin-list">${CABINS.map((cabin, i) => `<a class="cabin-card reveal" href="/cabanas/${cabin.slug}"><span class="cabin-index">0${i + 1}</span><span class="cabin-image-wrap"><img src="${cabin.image}" alt="${cabin.name}" loading="lazy"></span><span class="cabin-body"><span class="cabin-capacity">${cabin.capacity}</span><strong>${cabin.name}</strong><span class="cabin-arrow" aria-hidden="true">↗</span></span></a>`).join('')}</div><p class="micro-note reveal">Check-in ${BUSINESS.cabinCheckIn} · Check-out ${BUSINESS.cabinCheckOut}</p></section>

    <section class="events-feature section-pad"><div class="events-image reveal"><img src="${ASSETS.event}" alt="Espacio natural para eventos" loading="lazy"></div><div class="events-copy reveal"><p class="eyebrow">Momentos para compartir</p><h2>Celebra entre montañas.</h2><p>El espacio puede consultarse para reuniones, celebraciones, actividades familiares, grupos y eventos privados.</p><p class="events-note">La disponibilidad, condiciones y alcance se revisan directamente para cada solicitud.</p>${arrowLink('/eventos', 'Conocer eventos')}</div></section>

    <section class="flavor section-pad"><div class="flavor-copy reveal"><p class="eyebrow">Sabores de la finca</p><h2>Del campo a la mesa.</h2><p>Conoce las opciones de comidas bajo reservación y los productos lácteos artesanales de Finca Las Nubes.</p>${arrowLink('/sabores', 'Explorar sabores')}</div><div class="meal-times reveal"><div class="meal-block"><span class="meal-number">01</span><div><h3>Desayunos y almuerzos</h3><p>Sábados y domingos, bajo reservación.</p><strong>Consulta disponibilidad</strong></div></div><div class="meal-block"><span class="meal-number">02</span><div><h3>Lácteos artesanales</h3><p>Productos elaborados con leche de la finca.</p><strong>Ver productos</strong></div></div></div></section>

    <section class="reservation section-pad"><p class="reservation-banner"><span class="dot"></span>TODOS LOS SERVICIOS SE REALIZAN BAJO RESERVACIÓN.</p><div class="reservation-head"><p class="eyebrow light">Antes de venir</p><h2>Planea tu visita con calma.</h2><p>Escríbenos por WhatsApp para confirmar disponibilidad, condiciones y ruta.</p></div><div class="steps"><div class="step"><span>01</span><h3>Elige</h3><p>Cabaña, camping, pase de día, comidas o evento.</p></div><div class="step"><span>02</span><h3>Consulta</h3><p>Recibe información actualizada de la finca.</p></div><div class="step"><span>03</span><h3>Confirma</h3><p>Coordina tu visita directamente.</p></div><div class="step"><span>04</span><h3>Llega</h3><p>Consulta la ruta antes de salir.</p></div></div></section>

    <section class="location section-pad"><div class="location-copy reveal"><p class="eyebrow">Dónde estamos</p><h2>El camino también es parte de la experiencia.</h2><address>${BUSINESS.location.join('<br>')}</address><div class="route-note"><span class="route-icon">⌁</span><p>Consulta la ruta antes de salir. El último tramo incluye terracería.</p></div>${arrowLink('/como-llegar', 'Ver cómo llegar')}</div><div class="map-art reveal"><div class="map-rings"></div><span class="map-road road-one"></span><span class="map-road road-two"></span><span class="map-label label-city">San José Pinula</span><span class="map-label label-finca"><i class="pin"></i> Finca Las Nubes</span></div></section>

    <section class="final-cta section-pad"><p class="eyebrow light">Tu próxima escapada</p><h2>Empieza entre las <em>nubes.</em></h2><p>Consulta disponibilidad para cabañas, camping, pase de día, comidas o eventos.</p><div class="final-actions">${waButton('general', 'Consultar disponibilidad', 'button button-accent button-large')}${waButton('general', 'Escribir por WhatsApp', 'button button-ghost button-large')}</div><p class="phone">${BUSINESS.displayPhone}</p></section>
  </div>`;
}
